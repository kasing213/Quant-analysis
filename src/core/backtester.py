...
class MovingAverageStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        try:
            ...
            # Skip this iteration if data access fails
            pass
    ...
    def next(self):
        ...
        try:
            ...
            if not self.position:
                ...
                if (current_price > upper_band or  # Price above upper band
                    (self.buy_price and current_price <= self.buy_price * (1 - self.params.stop_loss)) or
                    (self.buy_price and current_price >= self.buy_price * (1 + self.params.take_profit))):
                    self.order = self.sell()
        except (IndexError, TypeError, AttributeError) as e:
...
class PortfolioBacktester:
    ...
    def get_data(self, symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        try:
            # Clean the symbol - ensure it's a string
            if isinstance(symbol, (tuple, list)):
                symbol = str(symbol[0]) if symbol else "SPY"
            else:
                symbol = str(symbol).strip()

            # Handle special cases for forex symbols
            if symbol.startswith("$"):
                symbol = symbol[1:]  # Remove $ prefix
            if "EURUSD" in symbol.upper():
                symbol = "EURUSD=X"  # Use proper Yahoo Finance forex format
            ...
            for col in required_columns:
                if col not in data.columns:
                    if col == 'Volume' and 'Adj Close' in data.columns:
                        data[col] = 0  # Set volume to 0 for forex pairs
                    else:
    ...
    def plot_results(self, symbol: str, save_path: str = None) -> str:
        ...
        try:
            plot_result = result.cerebro.plot(style='candlestick', barup='green', bardown='red')
            if plot_result and len(plot_result) > 0 and len(plot_result[0]) > 0:
                fig = plot_result[0][0]
            else:
                # Create simple matplotlib plot as fallback
                import matplotlib.pyplot as plt
